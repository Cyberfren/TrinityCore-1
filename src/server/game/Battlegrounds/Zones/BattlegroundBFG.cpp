...
void BattlegroundBFG::FillInitialWorldStates(WorldPackets::WorldState::InitWorldStates& packet)
{
    ...
    for (uint8 node = 0; node < BG_BFG_DYNAMIC_NODES_COUNT; ++node)
        packet.Worldstates.emplace_back(BG_BFG_OP_NODEICONS[node], (m_Nodes[node] == 0) ? 1 : 0);

    // Node occupied states
    for (uint8 node = 0; node < BG_BFG_DYNAMIC_NODES_COUNT; ++node)
        for (uint8 itr = 1; itr < BG_BFG_DYNAMIC_NODES_COUNT; ++itr)
            packet.Worldstates.emplace_back(BG_BFG_OP_NODESTATES[node] + plusArray[itr], (m_Nodes[node] == itr) ? 1 : 0);
    ...
    packet.Worldstates.emplace_back(BG_BFG_OP_OCCUPIED_BASES_ALLY, ally);
    packet.Worldstates.emplace_back(BG_BFG_OP_OCCUPIED_BASES_HORDE, horde);

    // Team scores
  packet.Worldstates.emplace_back(BG_BFG_OP_RESOURCES_MAX, BG_BFG_MAX_TEAM_SCORE);
   packet.Worldstates.emplace_back(BG_BFG_OP_RESOURCES_WARNING, BG_BFG_WARNING_NEAR_VICTORY_SCORE);
    packet.Worldstates.emplace_back(BG_BFG_OP_RESOURCES_ALLY, m_TeamScores[TEAM_ALLIANCE]);
    packet.Worldstates.emplace_back(BG_BFG_OP_RESOURCES_HORDE, m_TeamScores[TEAM_HORDE]);

    // other unknown BG_BFG_UNK_01
...
void BattlegroundBFG::AddPlayer(Player* player)
{
    ...
    if (!isInBattleground)
        PlayerScores[player->GetGUID().GetCounter()] = new BattlegroundBFGScore(player->GetGUID());
}

void BattlegroundBFG::RemovePlayer(Player* , ObjectGuid , uint32)
{
}
...
void BattlegroundBFG::EventPlayerClickedOnFlag(Player* source, GameObject*)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
